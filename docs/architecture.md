...
- `config.matcher` excludes anything starting with `api`, `_next/static`, `_next/image`, `favicon.ico` and common image extensions. `src/app/api/*` is never behind the proxy: `/api/mission-worker` is protected by a bearer token, `/api/phash` has no explicit auth check (it relies on the cookie-bound client and RLS).
...
- Env vars are listed in `.env.example` and `supabase/.env.example`. Read by code but absent from `.env.example`: `MISSION_RUN_INLINE`, `NEXT_PUBLIC_APP_URL`, `YUPOO_DISCOVERY_CONCURRENCY`, `UI_PREVIEW_MODE`, `MISSION_WORKER_TIMEOUT_MS` (only in `supabase/.env.example`).
...
- Auth concern (unverified at runtime; derived from code and migrations): the route uses the cookie-bound `createClient()`, the trigger `fetch` forwards no cookies or token, `/api` is outside the proxy, `photo_hashes` has no UPDATE policy, and `similarity_matches` has no INSERT policy. Server-to-server triggers therefore run as an anonymous client, and even authenticated calls would have their `photo_hashes` update affect zero rows and their `similarity_matches` insert rejected. Verify end to end before relying on this path.
